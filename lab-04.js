...
fastify.get("/name", (request, reply) => {
  //(A) Gettting information from the requester
 const {first,last} = request.query;


 //const name //if first and last  are defined, the name = "Alex Salazar" else "Guest"
...
// (C) Using the tranformed data to inform our response to the client
  reply
    .code(200)
    .header("Content-Type", "text/html; charset=utf-8")
    .send(`<h1>${name}</h1>`);
